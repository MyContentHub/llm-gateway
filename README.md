...
- **管理仪表盘** — React + shadcn/ui 暗色主题管理界面，7 个页面（概览、密钥、审计、安全、提供商、设置）
...
- **OpenAPI 文档** — 自动生成的 Swagger UI 交互式 API 文档（`/docs`）
- **审计日志自动清理** — 可配置保留天数和 cron 定时策略，自动清理过期日志和请求体
...
网关内置 React 管理界面，启动后访问 `http://localhost:3000/admin`：

```bash
cd admin
pnpm install          # 安装前端依赖
pnpm build            # 构建前端（输出到 admin/dist/）
```

构建后重启网关即可自动加载管理界面。开发模式下前端开发服务器代理 API 请求到网关：

```bash
cd admin && pnpm dev  # 前端开发服务器 http://localhost:5173
```

管理界面使用 `config.toml` 中的 `admin_token` 登录。
...
| 接口 | 方法 | 说明 |
|------|------|------|
| `/api/v1/chat/completions` | POST | Chat 补全（支持流式 / 非流式） |
| `/api/v1/embeddings` | POST | 文本向量化 |
| `/api/v1/models` | GET | 列出所有已配置提供商的可用模型 |
...
| 接口 | 方法 | 说明 |
|------|------|------|
| `/api/admin/keys` | POST | 创建虚拟 API Key |
| `/api/admin/keys` | GET | 列出所有 Key（分页） |
| `/api/admin/keys/:id` | GET | 获取单个 Key 详情 |
| `/api/admin/keys/:id` | PATCH | 更新 Key（名称、速率限制） |
| `/api/admin/keys/:id` | DELETE | 吊销 Key |
| `/api/admin/audit/logs` | GET | 查询审计日志（分页、过滤） |
| `/api/admin/audit/logs/:requestId` | GET | 获取单条审计日志详情 |
| `/api/admin/audit/stats` | GET | 审计统计信息 |
| `/api/admin/audit/security` | GET | 安全统计（拦截、PII、注入、内容过滤） |
| `/api/admin/config` | GET | 获取运行时配置（脱敏） |
| `/api/admin/providers` | GET | 列出所有提供商（含 Key 数量） |
| `/api/admin/providers/health` | GET | 提供商 Key 健康状态 |
...
| `body_retention_days` | `7` | 请求/响应体保留天数 |
| `cleanup_cron` | `"0 * * * *"` | 清理定时任务 cron 表达式 |
