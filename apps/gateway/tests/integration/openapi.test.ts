...
describe("OpenAPI Integration", () => {
  ...
  it("documents all v1 endpoints with VirtualKey security", async () => {
    ...
    try {
      ...
      const chatCompletions = spec.paths["/api/v1/chat/completions"];
      ...
      const embeddings = spec.paths["/api/v1/embeddings"];
      ...
      const models = spec.paths["/api/v1/models"];
  ...
  it("documents all admin key endpoints with AdminToken security", async () => {
    ...
    try {
      ...
      const postKeys = spec.paths["/api/admin/keys"].post;
      ...
      const getKeys = spec.paths["/api/admin/keys"].get;
      expect(getKeys.security).toEqual([{ AdminToken: [] }]);
      expect(getKeys.parameters).toBeDefined();

      const getKey = spec.paths["/api/admin/keys/{id}"];
  ...
  it("documents all admin audit endpoints with AdminToken security", async () => {
    ...
    try {
      ...
      const auditLogById = spec.paths["/api/admin/audit/logs/{requestId}"];
      expect(auditLogById).toBeDefined();
      expect(auditLogById.get.security).toEqual([{ AdminToken: [] }]);

      const auditStats = spec.paths["/api/admin/audit/stats"].get;
  ...
  it("documents all expected paths in the spec", async () => {
    ...
    try {
      ...
      expect(paths).toContain("/api/v1/embeddings");
      expect(paths).toContain("/api/v1/models");
      expect(paths).toContain("/api/admin/keys");
      expect(paths).toContain("/api/admin/keys/{id}");
      expect(paths).toContain("/api/admin/audit/logs");
      expect(paths).toContain("/api/admin/audit/logs/{requestId}");
      expect(paths).toContain("/api/admin/audit/stats");
